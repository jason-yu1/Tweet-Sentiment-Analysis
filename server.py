...
def tweets(name):
    ...
    # Get median sentiment score
    m = median([tweet['score'] for tweet in tweets], axis=0)
    
    # Add colors to tweets
    add_color(tweets=tweets)
    
    return render_template('tweets.html', record=record, m = m, tweets=tweets, user=name)
